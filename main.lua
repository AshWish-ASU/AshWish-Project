...
task.spawn(function()
    ...
    local function forceRejoin() 
        if reconnecting then return end; reconnecting = true
        SendEmergencyPing("DISCONNECT - REJOINING", "Error detected.", tonumber(0xFFA500)); WriteHopState()
...
local function SendUserWebhook()
    ...
    local embeds = {}
    if not hasSentUserExecutionLog then table.insert(embeds, {["title"]="INIT AshWish Script Executed", ["color"]=tonumber(0x00AFFF), ["fields"]={{["name"]="Player", ["value"]=localPlayer.Name, ["inline"]=true}, {["name"]="Executions", ["value"]=tostring(execCount), ["inline"]=true}, {["name"]="Device", ["value"]=isPC and "PC" or "Mobile", ["inline"]=true}, {["name"]="Time Used", ["value"]=GetTimeUsedString(), ["inline"]=false}}, ["footer"]={["text"]="Execution Log - "..os.date("%I:%M:%S %p")}}); hasSentUserExecutionLog = true end
    table.insert(embeds, {["title"]="Auto Farm Stats", ["color"]=tonumber(0x00FF00), ["fields"]={{["name"]="Player", ["value"]=localPlayer.Name, ["inline"]=false}, {["name"]="Current Level", ["value"]=FormatNum(curLvl), ["inline"]=true}, {["name"]="Levels Gained", ["value"]="+"..FormatNum(lvlGain), ["inline"]=false}, {["name"]="Levels/Min", ["value"]=FormatNum(lpm), ["inline"]=true}, {["name"]="Levels/Hr", ["value"]=FormatNum(lph), ["inline"]=true}, {["name"]="Peak Levels / Hr", ["value"]=FormatNum(PeakLevelsPerHour), ["inline"]=true}, {["name"]="Status", ["value"]=isFarming and "Active" or "Idle", ["inline"]=false}}, ["footer"]={["text"]="AshWish UI - "..os.date("%I:%M:%S %p")}})
    local httprequest = (syn and syn.request) or (http and http.request) or http_request or request
